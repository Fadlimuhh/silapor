...
@section('title', 'SiLapor | Edit Masyarakat')
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            <h1 class="h3 mb-0 text-gray-800">Edit Masyarakat</h1>
            ...
            <form action="{{ route('masyarakat.update', $masyarakat->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control" required>
                        <option value="aktif" {{ old('status', $masyarakat->status) == 'aktif' ? 'selected' : '' }}>
                            Aktif
                        </option>

                        <option value="nonaktif" {{ old('status', $masyarakat->status) == 'nonaktif' ? 'selected' : '' }}>
                            Nonaktif
                        </option>

                        <option value="belum_verifikasi" {{ old('status', $masyarakat->status) == 'belum_verifikasi' ? 'selected' : '' }}>
                            Belum Verifikasi
                        </option>
                    </select>
                </div>
